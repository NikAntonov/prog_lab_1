...
void output() {                                                                                                         //Обработка функции и вывод значения
    ...
    for (double i = x_start; i <= x_end; i+=abs(dx)) {                                                                  //Прогон цикла по отрезку функции с нужным шагом
        ...
        if (result > 0) result_forRome = floor(result + 0.5); else result_forRome = ceil(result - 0.5);                 //Округление до целого значения

        cout<<"["<<i<<"; "<<result<<"] -> (";                                                                           //Печать результата в арабских цифрах

        if (i < 0) cout<<"-";                                                                                           //Проверка на отрицательность
        print(int(abs(i)));                                                                                             //Печать римских цифр
        cout<<"; ";
        if (result < 0) cout<<"-";                                                                                      //Проверка на отрицательность
        print(int(abs(result_forRome)));                                                                                //Печать римских цифр
...
int main(){
    ...
    if (dx == 0) {
        cout<<"ERROR: null dx"<<endl;                                                                                   //Обработка ошибки с бесконечным циклом
